...
set -e
cd "$(dirname "$0")"
cat src/game_core.js src/game_town.js src/game_logic.js src/game_quests.js > .build_check.js
...
rm -f .build_check.js
{
  ...
  echo '<script>'
  cat src/game_core.js src/game_town.js src/game_logic.js src/game_quests.js
  echo '</script>'
} > index.html
